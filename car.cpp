...
void DynamicStatusUpdate() {
		...
		if (car[i].FindAvoid == 1) {
			...
			while (car[firstnum].FindAvoid) {
				...
				if (car[i].ObCheck(car[firstnum].x, car[firstnum].y, car[i].x, car[i].y, 51, 0, 0) &&
					Dot(car[firstnum].x - car[i].x, car[firstnum].y - car[i].y, des[i].first - car[i].x, des[i].second - car[i].y) >= 0) {
					car[i].FindAvoid = 0;
					car[i].Reach = false;
					car[i].lassta3 = car[i].lassta2 = car[i].lassta = make_pair(0.0, 0.0);
					car[i].Avoidnum = -1;
	...
	for (int i = 0; i < 4; i++) {
		if (car[i].FindAvoid == 1) {
			...
			output << "I'm reversing" << endl << endl;
			int numm = i, len = 0;
			st[len = 1] = numm;
...
void IntoDynamicCheckAndUpdate() {
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < 4; j++) {
			...
			bool check3 = (wait_until_spare_7[i] || wait[i]) &&
				Dot(vec2, Sub(make_pair(car[i].x, car[i].y), make_pair(car[j].x, car[j].y))) > 0 &&
				PointToLine(make_pair(car[i].x, car[i].y), make_pair(car[j].x, car[j].y), vec2) < car[i].GetR(car[i].goods) + car[j].GetR(car[j].goods)&&
				Dist(car[i].x, car[i].y, car[j].x, car[j].y) < 3 &&
				(car[i].Search(car[i].x, car[i].y, 51, car[i].GetR(car[i].goods) + 2.0 * car[j].GetR(car[j].goods) + 0.03) || car[j].Search(car[j].x, car[j].y, 51, 2.0 * car[i].GetR(car[i].goods) + car[j].GetR(car[j].goods) + 0.03));
...
bool Car::CheckCar(double x, double y) {
	for (int i = 0; i < 4; i++) {
		if (Dist(x, y, car[i].x, car[i].y) < car[i].GetR(car[i].goods) + 0.2)
			return false;
	}
	return true;
}
...
void Car::SetRFforInactive(int numID, int Choosenum, double& forwar, double& rot) {
	...
	if (d > GetR(goods) + GetR(car[Choosenum].goods) - 0.05 /* && ((uy > eps && (d - GetR(goods) - GetR(car[Choosenum].goods)) / uy >= CheckTime) || uy <= eps)*/)
		forwar = 6;
	else forwar = 6 * cos((1 - max(Dist(x, y, car[Choosenum].x, car[Choosenum].y) - GetR(goods) - GetR(car[Choosenum].goods + 3), 0.0) / (AlertRange - GetR(goods) - GetR(car[Choosenum].goods))) * (Pi / 2));
	...
	*/
	...
	if (checknewpo1) {
		...
		pair<double, double>newvec = Sub(newpo1, make_pair(x, y));
		pair<double, double>desvec = Sub(des[numID], make_pair(x, y));
		if (Cross(desvec, newvec) <= 0)
	...
	//前往左侧
		...
		pair<double, double>newvec = Sub(newpo2, make_pair(x, y));
		pair<double, double>desvec = Sub(des[numID], make_pair(x, y));
		if (Cross(newvec, desvec) <= 0)
...
void Car::SetRFforCrash(int numID, int Choosenum, double& forwar, double& rot) {
	...
	if ((car[Choosenum].FindAvoid && car[Choosenum].Reach) || (fabs(car[Choosenum].vx) < 0.1 && fabs(car[Choosenum].vy) < 0.1))
...
bool Car::ChooseAvoider(int Cnum) {
	...
	if(wait_until_spare_7[FindnumID()] || wait[FindnumID()])
		return false;
	...
	if (!car[Cnum].FindAvoid && ((goods > car[Cnum].goods || (goods == car[Cnum].goods && numID > Cnum))||(wait_until_spare_7[Cnum])))
		return true;
...
pair<double, double> Car::Dynamic_Avoidance() {
	...
	while (startang < endang) {
		...
		//判定该点是否可以作为避让点
		if (AvoidSituation1Check(sx + maxlen * cos(startang), sy + maxlen * sin(startang)) && CheckCar(sx + maxlen * cos(startang), sy + maxlen * sin(startang))) {
	...
	while (startang < endang) {
		...
		*/
		...
		if (fabs(goodPoint.first + 1) > eps && Dist(goodPoint, realT) > 2.0 * GetR(car[Avoidnum].goods) + 0.03 && 
			CheckCar(x + (Dist(realS, goodPoint) + 2.0 * GetR(car[Avoidnum].goods) + 0.03) * cos(startang),
...
void calc() {
	...
	output << "car[3].FindAvoid=" << car[3].FindAvoid << endl << endl;
